...
export default function ProjectDocumentsTab({ project }: { project: any }) {
  ...
  const [versionTargetDelivId, setVersionTargetDelivId] = useState<
    string | null
  >(null);
  ...
  const [reviewStatus, setReviewStatus] = useState<DeliverableStatus>(
  ...
  async function handleAddDocument() {
    ...
    try {
      if (uploadType === "file") {
        ...
        const uploadInfo = await performPhysicalUpload(generalFile);
        finalUrl = uploadInfo.fileUrl;
      } else if (!fileUrl.trim()) {
        alert("URL é obrigatória para o tipo Link.");
        setIsLoading(false);
        return;
      }

      await uploadProjectDocument(project.id, finalUrl, fileName);
      setFileName("");
      setFileUrl("");
      setGeneralFile(null);
  ...
  async function performPhysicalUpload(fileToUpload: File): Promise<any> {
    ...
    if (!uploadResult.ok) {
      throw new Error(
        uploadResult.error || "Falha no upload do arquivo físico.",
      );
    }
  ...
  async function handleCreateDeliverable() {
    ...
    try {
      ...
      if (!taskId) {
        alert(
          "Para criar um entregável, o projeto precisa de pelo menos uma tarefa ativa no Kanban.",
        );
        setIsLoading(false);
        return;
      }
  ...
  async function handleDeleteDeliverable(id: string) {
    if (!confirm("Tem certeza que deseja excluir este entregável técnico?"))
      return;
    setIsLoading(true);
    try {
  ...
  const getStatusBadge = (status: DeliverableStatus) => {
    ...
    return (
      <Badge
        className={`${details.style} border-none font-bold rounded-lg px-2.5 py-0.5 text-xs`}
      >
        {details.text}
      </Badge>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-gray-100 dark:border-slate-800">
        <div className="bg-muted p-1 rounded-xl flex gap-1 w-full sm:w-auto">
          <button
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Button
                ...
                variant={isAddingDoc ? "outline" : "default"}
              ...
              <Card className="rounded-2xl border-border shadow-lg">
                <CardContent className="pt-6 grid gap-4">
                  <div className="flex gap-2 border-b border-border pb-4">
                    <Button
                      variant={uploadType === "file" ? "secondary" : "ghost"}
                      size="sm"
                      onClick={() => setUploadType("file")}
                      className="rounded-lg h-8 text-xs font-semibold"
                    >
                      <Upload className="h-3.5 w-3.5 mr-1.5" /> Arquivo Local
                    </Button>
                    <Button
                      variant={uploadType === "link" ? "secondary" : "ghost"}
                      size="sm"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {uploadType === "link" ? (
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="border border-dashed border-border rounded-xl p-2.5 relative bg-muted/50 hover:border-primary/50 transition-colors">
                          <input
                            type="file"
                          ...
                          />
                          <div className="text-sm font-medium text-muted-foreground truncate flex items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={isLoading}
                    className="w-full sm:w-auto ml-auto rounded-xl mt-2"
                  >
                    Salvar Documento
                  </Button>
                </CardContent>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50 border-b border-border">
                    <TableHead className="py-3 px-4 font-bold text-xs uppercase text-muted-foreground">
                      Nome
                    </TableHead>
                    <TableHead className="py-3 px-4 font-bold text-xs uppercase text-muted-foreground hidden sm:table-cell">
                      Tipo
                    </TableHead>
                    <TableHead className="py-3 px-4 font-bold text-xs uppercase text-muted-foreground hidden md:table-cell">
                      Data de Envio
                    </TableHead>
                    <TableHead className="py-3 px-4 font-bold text-xs uppercase text-muted-foreground text-right">
                      Ações
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {filteredDocuments.map((doc, index) => (
                    ...
                    >
                      ...
                      <TableCell className="py-4 px-4 hidden sm:table-cell">
                        <span className="text-[10px] px-2 py-0.5 bg-muted text-muted-foreground rounded-md font-bold uppercase">
                          {getFileType(doc.name)}
                        </span>
                      </TableCell>
                      <TableCell className="py-4 px-4 hidden md:table-cell text-muted-foreground text-sm">
                        {doc.uploadedAt
            ...
            <Card className="border-2 border-dashed border-primary/20 bg-primary/5 p-6 rounded-2xl text-center">
              <CardContent className="p-0 flex flex-col items-center justify-center space-y-3">
                ...
                <p className="text-xs text-muted-foreground px-4">
                  Esta aba armazena links rápidos de referência geral do
                  projeto, como planilhas compartilhadas no Drive ou contratos.
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="md:col-span-2 space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Button
                ...
                variant={isAddingDeliverable ? "outline" : "default"}
                className="rounded-xl"
              >
            ...
            {isAddingDeliverable && (
              <Card className="rounded-2xl border-border shadow-xl overflow-hidden animate-in fade-in duration-300">
                <CardContent className="pt-6 grid gap-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        onValueChange={(val) =>
                          setDelivType(val as DeliverableType)
                        }
                      ...
                      >
                        ...
                        <SelectContent className="rounded-xl border-border">
                          <SelectItem value={DeliverableType.SKETCH}>
                            Esboço (Sketch)
                          </SelectItem>
                          <SelectItem value={DeliverableType.RENDER_3D}>
                            Renderização 3D
                          </SelectItem>
                          <SelectItem value={DeliverableType.DRAWING_2D}>
                            Desenho Técnico 2D (CAD/BIM)
                          </SelectItem>
                          <SelectItem value={DeliverableType.PDF}>
                            Arquivo PDF
                          </SelectItem>
                          <SelectItem value={DeliverableType.DOCUMENT}>
                            Documento / Memorial
                          </SelectItem>
                    ...
                    <div className="space-y-2 sm:col-span-2">
                      ...
                      <Input
                        placeholder="Breve descrição sobre esta versão do entregável..."
                  ...
                  {/* Upload Físico Zone */}
                  <div className="border border-dashed border-border rounded-xl p-3 relative bg-muted/50 hover:border-primary/50 transition-colors">
                    <input
                      type="file"
                      className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                      onChange={(e) => {
                    ...
                    {delivFile ? (
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1">
                          {(delivFile.size / (1024 * 1024)).toFixed(2)} MB
                        </p>
                      </div>
                    ) : (
                      <div>
                        <p className="text-sm font-medium">
                          Clique ou arraste o arquivo físico aqui
                        </p>
                        <p className="text-xs text-slate-400 mt-1">
                          Formatos suportados: DWG, PDF, PNG, JPG, RVT, PLN (Max
                          15MB)
                        </p>
                      </div>
                    )}
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <Button
                      ...
                      disabled={isLoading}
                      className="rounded-xl px-6 bg-primary hover:bg-primary/95 text-background-dark font-bold shadow-lg shadow-primary/20"
                    >
                      Fazer Upload & Salvar
                    </Button>
                  </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50 border-b border-border">
                    ...
                    <TableHead className="py-3 px-4 font-bold text-xs uppercase text-muted-foreground text-center">
                      Versão
                    </TableHead>
                    <TableHead className="py-3 px-4 font-bold text-xs uppercase text-muted-foreground">
                      Status
                  ...
                  {filteredDeliverables.map((deliv) => (
                    ...
                    >
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-muted-foreground font-bold block mt-0.5">
                              {deliv.mimeType || "CAD / BIM"} •{" "}
                              {(deliv.fileSize / (1024 * 1024)).toFixed(1)} MB
                            </span>
                          </div>
                        </div>
                      </TableCell>
                      <TableCell className="py-4 px-4 text-center font-bold">
                        <Badge
                          variant="outline"
                          className="rounded-lg bg-muted font-bold"
                        >
                          v{deliv.version}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="py-4 px-4 hidden sm:table-cell text-muted-foreground font-medium text-sm">
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {deliv.status === "DRAFT" && (
                            <Button
                              ...
                              size="sm"
                              className="rounded-lg text-xs font-bold bg-primary hover:bg-primary/95 text-primary-foreground"
                              disabled={isLoading}
                            >
                              Enviar p/ Revisão
                            </Button>
                          ...
                          {(deliv.status === "REJECTED" ||
                            deliv.status === "APPROVED_WITH_CHANGES") && (
                            <Dialog
                              open={versionTargetDelivId === deliv.id}
                              onOpenChange={(open) =>
                                setVersionTargetDelivId(open ? deliv.id : null)
                              }
                            >
                              ...
                              <DialogContent className="rounded-2xl border-border">
                                <DialogHeader>
                                  ...
                                  <DialogDescription className="text-sm">
                                    ...
                                    corrigida do entregável{" "}
                                    <strong className="text-foreground">
                                      {deliv.name}
                                    </strong>
                                    . A versão será automaticamente incrementada
                                    para v{deliv.version + 1}.
                                  </DialogDescription>
                                </DialogHeader>
                                <div className="space-y-4 py-4">
                                  <div className="border-2 border-dashed border-border p-6 rounded-2xl text-center relative cursor-pointer hover:border-primary/50 transition-colors">
                                    <input
                                      type="file"
                                      className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                                      onChange={(e) => {
                                    ...
                                    {versionFile ? (
                                      ...
                                      <p className="text-xs text-muted-foreground">
                                        Arraste ou selecione a nova revisão
                                        física
                                      </p>
                                    )}
                                  </div>
                                ...
                                <DialogFooter className="gap-2">
                                  ...
                                  <Button
                                    ...
                                    disabled={isLoading || !versionFile}
                                    className="rounded-xl px-6 bg-primary hover:bg-primary/95 text-background-dark font-bold"
                                  >
                                    Submeter Revisão
                                  </Button>
                                </DialogFooter>
              ...
              <CardContent className="p-5 space-y-4">
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400 space-y-2.5">
                  <div className="flex items-start gap-2">
                    ...
                    <p>
                      <strong>Rascunho (DRAFT)</strong>: Arquivo pode ser
                      editado ou excluído. Envie para revisão ao concluir.
                    </p>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="h-1.5 w-1.5 rounded-full bg-blue-400 mt-1.5 shrink-0" />
                    <p>
                      <strong>Aguardando Revisão</strong>: Travado para edição
                      do arquiteto, aguardando aprovação do cliente.
                    </p>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 mt-1.5 shrink-0" />
                    <p>
                      <strong>Aprovado (APPROVED)</strong>: Travado
                      permanentemente contra modificação, alteração ou exclusão
                      física.
                    </p>
                  </div>
                  <div className="flex items-start gap-2">
                    ...
                    <p>
                      <strong>Rejeitado / Ajustes</strong>: O arquiteto pode
                      fazer correções e enviar a nova revisão incrementando a
                      versão.
                    </p>
                  </div>
                </div>
      ...
      {reviewTargetDeliv && (
        <Dialog
          open={!!reviewTargetDeliv}
          onOpenChange={(open) => {
            if (!open) setReviewTargetDeliv(null);
          }}
        >
          <DialogContent className="max-w-2xl rounded-2xl border-gray-200 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription className="text-sm">
                ...
                <strong className="text-slate-900 dark:text-slate-100">
                  {reviewTargetDeliv.name} (v{reviewTargetDeliv.version})
                </strong>
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 max-h-[250px] overflow-y-auto border border-gray-50 dark:border-slate-850 p-4 rounded-xl">
                  {reviewTargetDeliv.reviewComments &&
                  (reviewTargetDeliv.reviewComments as any[]).length > 0 ? (
                    (reviewTargetDeliv.reviewComments as any[]).map(
                      (rev: any, idx: number) => (
                        <div
                          key={rev.id || idx}
                          className="flex gap-3 text-sm items-start pb-4 border-b border-gray-50 dark:border-slate-800 last:border-b-0"
                        >
                          <div
                    ...
                    <p className="text-xs text-slate-400 text-center py-4">
                      Nenhuma interação ou feedback registrado para este
                      entregável técnico.
                    </p>
                  )}
                </div>
              ...
              {reviewTargetDeliv.status === "PENDING_REVIEW" ? (
                <div className="space-y-4 border-t border-gray-100 dark:border-slate-800 pt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setReviewStatus(DeliverableStatus.APPROVED_WITH_CHANGES)
                      }
                      className={`rounded-xl font-bold ${
                        reviewStatus === DeliverableStatus.APPROVED_WITH_CHANGES
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setReviewStatus(DeliverableStatus.REJECTED)
                      }
                      className={`rounded-xl font-bold ${
                        reviewStatus === DeliverableStatus.REJECTED
                ...
                <div className="p-4 bg-slate-50 dark:bg-slate-900/30 border border-gray-100 dark:border-slate-800 rounded-xl text-center">
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">
                    Este entregável técnico está em status de{" "}
                    <strong>{reviewTargetDeliv.status}</strong> e não aguarda
                    avaliações de revisão ativa.
                  </p>
                </div>
              )}
            </div>
            ...
            <DialogFooter className="gap-2 border-t border-gray-100 dark:border-slate-800 pt-4">
              <Button
                variant="outline"
                className="rounded-xl"
                onClick={() => setReviewTargetDeliv(null)}
              >
                Fechar
              </Button>
              {reviewTargetDeliv.status === "PENDING_REVIEW" && (
